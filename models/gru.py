"""Plain unidirectional GRU baseline for structured flow-token prediction."""
...
def build_gru_model(cfg: Any, vocab_sizes: Dict[str, int]) -> Model:
    inputs, x, sizes = build_token_encoder(cfg, vocab_sizes)
    dropout = float(cfg_value(cfg, "dropout", 0.3))
    units_list = parse_units(cfg_value(cfg, "gru_units", None), parse_units(cfg_value(cfg, "lstm_units", None), [128, 64]))
    ...
    return Model(inputs=inputs, outputs=prediction_heads(x, sizes), name="flow_gru")
